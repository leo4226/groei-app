...
Add a self-serve password reset flow to the Groei login page. Users receive a reset link by email (via Resend), click it, and set a new password. Targets real external users beyond the Leon & Lisbeth household.
...
- HTML: inline string, Groei branding (Fraunces font for the title, green CTA button), 1-hour expiry note, ignore-if-not-you footer.
