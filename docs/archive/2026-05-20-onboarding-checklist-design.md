...
New users landing on the dashboard for the first time see an empty, confusing screen. This spec adds a dismissible welcome checklist card to the dashboard that guides them through the two steps needed before Floreren is useful: creating a map and adding a first plant.
