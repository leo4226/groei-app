...
Add `dashboardV2: DashboardV2Response | null` and `loadDashboardV2()` to `useFloreren`. Keep existing `dashboard` and `loadDashboard` — used by other pages if any. `Dashboard.tsx` switches to `dashboardV2`.
