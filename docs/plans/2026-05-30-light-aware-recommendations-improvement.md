...
**Date:** 2026-05-30
**Status:** Steps A, B & C done (2026-05-30). LLM provider switched to OpenRouter and the `sun_preference` backfill ran on prod (77% coverage). Remaining follow-up: `getSunFit`/`PLANT_SUN_PROFILES` retune.
...
- **Step A (backfill) — DONE 2026-05-30.** Before: **1741 of 1743 enriched species had NULL `sun_preference`** (light model running entirely on the `tolerated` lane). The DeepSeek account was out of credit (`402`), so the LLM provider was switched to **OpenRouter** (see `backend/llm_config.py`, model `deepseek/deepseek-chat`). Added a surgical `--only-missing-sun` mode to `scripts/enrich_species_ecology.py` that asks the LLM for sun_preference only and writes just that one column (no GBIF re-derivation, idempotent). Ran on prod: **1339 filled, 398 left NULL (LLM unsure), 0 errors.** New distribution: full_sun 33%, partial_sun 29%, shade 13%, any 2%, NULL 23% → **77% now have a known preference**. The remaining 23% stay in the `tolerated` lane; re-run `--only-missing-sun` later to retry them. **Prod serving still needs the Fly secret:** `flyctl secrets set OPENROUTER_API_KEY=<key> -a floreren-api --remote-only`.
