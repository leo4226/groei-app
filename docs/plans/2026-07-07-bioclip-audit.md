...
- **Validate §3.1 before merge — one command:** `python scripts/eval_blend.py` (needs the worker + eval photos, no prod DB). It uses leave-one-out (each photo tested with all others as confirmed refs) to report (a) same-species vs different-species image-cosine distributions → where to set the floor, and (b) text-only vs blended top-1/top-5. Confirm blended beats text-only and that `_IMAGE_MATCH_MIN` sits between the two distributions so look-alikes aren't wrongly rescued; adjust the constant in `routers/plant_id.py`.
