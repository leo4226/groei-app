...
async def sync_ephemeral_schedules(db=None) -> dict:
    ...
    Pass an existing `db` to reuse the caller's connection (e.g. the digest
    cron, which already holds one); omit it to open a short-lived connection of
    our own (the dashboard call site). Returns summary: {created, deleted}.
    """
    if db is None:
