...
const tabs = [
  ...
  },
  {
    ...
    label: 'Add',
    icon: () => (
      <div className="w-10 h-10 -mt-4 rounded-full bg-primary text-white flex items-center justify-center shadow-lg">
      ...
      {tabs.map((tab) => (
        ...
        >
          {({ isActive }) => (
            <>
              {tab.icon(isActive)}
              <span className={tab.to === '/plants/add' ? '-mt-1' : ''}>{tab.label}</span>
            </>
          )}
        </NavLink>
      ))}
