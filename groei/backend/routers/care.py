...
async def mark_care_done(action: CareAction):
...
async def skip_care(action: CareAction):
    async with get_db() as db:
        cursor = await db.execute(
            """SELECT id, interval_days, season_adjust FROM care_schedules
               WHERE plant_id = ? AND care_type = ? AND is_active = 1""",
...
async def delete_care_schedule(schedule_id: int):
    async with get_db() as db:
        cursor = await db.execute(
...
async def get_care_log(plant_id: int):
    async with get_db() as db:
        cursor = await db.execute(
